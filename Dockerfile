...
RUN { \
    ...
    echo 'else'; \
    echo '  while true; do' ; \
    echo '      if [[ ! -e block_server ]]; then' ; \
    echo '          echo "Starting AutoCode..."' ; \
    echo '          python src/autocode.py "${@}" || echo "AutoCode exited."'; \
    echo '      fi' ; \
    echo '      echo "Restarting AutoCode in 5 seconds...";' ; \
    echo '      sleep 5'; \
    echo '  done' ; \
    echo 'fi'; \
    } > /entrypoint.sh \
...
COPY /src/requirements.txt /opt/python/
